...
pub async fn resource_by_path_handler(
    ...
    let response = match load_file_into_response(file).await {
        ...
        Err(404) => (StatusCode::NOT_FOUND, helpers::err_template(404 as u32))
            .into_response(),
        _ => (
...
pub async fn secret_handler(
    ...
    if token != expected_token {
        return (StatusCode::UNAUTHORIZED, helpers::err_template(401 as u32))
            .into_response();
    } else {
        let html_content = r#"
            <!DOCTYPE html>
...
pub async fn recursive_redirect_handler(
    ...
    if n == 1 {
        ...
        response::Redirect::permanent(&format!("/secret?token={}", token))
            .into_response()
    } else if n > 0 && n < 255 {
        response::Redirect::permanent(&format!("/recursive/{}", n - 1))
            .into_response()
    } else {
        println!("Bad request");
        (StatusCode::BAD_REQUEST, helpers::err_template(400 as u32))
            .into_response()
    }
}

pub async fn absolute_redirect_handler() -> impl IntoResponse {
    response::Redirect::permanent("http://www.pdf995.com/why.html")
        .into_response()
}
