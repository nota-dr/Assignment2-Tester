...
impl TestAgent for RelativeRedirectTimes {
    async fn validate(
        ...
        let response_headers_n_times: Vec<&str> =
            vec!["http/1.1 308 permanent redirect", "content-length: 0"];
        println!("{:?}", args);
        let times = args
            .last()
            .unwrap()
...
impl TestAgent for LifeCycle {
    async fn validate(
        ...
        let request_len =
            end_of_req_headers as i32 - start_of_req_headers as i32;
        if request_len < 0 {
            return false;
        }

        let request_printf =
            format!("LEN = {}", request_len as u64).into_bytes();
        ...
        let total_num_of_bytes: i32 =
            end_of_response as i32 - start_of_response as i32;
        if total_num_of_bytes < 0 {
            return false;
        }
        // allow students to be off by +-1 byte
        let summarize_printf_v1 =
            format!("bytes: {}", total_num_of_bytes as u64).into_bytes();
...
impl TestAgent for Valgrind {
    async fn validate(
        ...
        let absolute_redirect_valgrind =
            cwd.join("valgrind - Absolute Redirect");
        let relative_redirect_valgrind =
            cwd.join("valgrind - Relative Redirect N Times");
